...
print(f'Было обработано {hh_page_count} страниц выдачи hh.ru. Была собрана информация о {hh_vacancies_count} вакансиях')
...
for i in range(sj_pages_to_process):
    sj_html = requests.get(
        f'{sj_main_link}?keywords={vacancy_name}&page={i}',
        headers=header).text
    sj_soup = bs(sj_html, 'html.parser')
...
print(f'Было обработано {sj_page_count} страниц выдачи sj.ru. Была собрана информация о {sj_vacancies_count} вакансиях')

df = pd.DataFrame(vacancies_final_list)
